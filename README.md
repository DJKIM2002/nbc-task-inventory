...
동적 메모리 관리: new와 delete[]를 사용한 배열의 생성 및 소멸자(destructor) 처리
- 기본 동작 구현:
AddItem(): 아이템 추가
RemoveLastItem(): 마지막 아이템 삭제
PrintAllItems(): 저장된 모든 아이템 정보 출력
GetSize() / GetCapacity(): 멤버 변수의 현재 상태 확인

2. 도전 기능 및 추가 구현 (Challenge & Extra Features)
동적 크기 확장 (Dynamic Resizing)
Resize() 함수 구현: 인벤토리 용량을 동적으로 조절
AddItem() 개선: 인벤토리가 가득 찼을 때, 자동으로 용량(capacity)을 2배로 확장하여 데이터 유실 방지
깊은 복사 (Deep Copy) 지원
...
compareItemsByPrice 비교 함수를 정의하여 가격 오름차순으로 아이템 정렬 기능 구현

📸 실행 결과 (Screenshots/Logs)
[여기에 실행 결과 스크린샷을 넣어주세요]
